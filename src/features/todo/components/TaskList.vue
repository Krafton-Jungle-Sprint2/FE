      ...
      <section v-for="sec in sections" :key="sec.key" class="bg-gray-50 rounded-lg p-3">
        ...
        <div class="space-y-2">
          <TaskItem v-for="t in sec.items" :key="t.id" :task="t" @toggle="(id, c) => emit('toggle', id, c)"
            @dates="(id, payload) => emit('dates', id, payload)" @remove="id => emit('remove', id)" />
...
const emit = defineEmits(['toggle', 'dates', 'remove'])
...
const toItem = (t, label) => ({
  id: t.id,
  title: t.title,
  status: label === '기한 지남' || label === '오늘 할 일' || label === '내일 이후'
    ? t.status : t.status,
...
const sections = computed(() => {
  ...
  overdue.sort(by('dueDate')); todayArr.sort(by('dueDate')); future.sort(by('startDate'))
  return [
